...
def _oracle_create_db(cfg, eng, ident):
    ...
    # fail otherwise
    with eng.connect() as conn:
        conn.execute("create user.txt %s identified by xe" % ident)
        conn.execute("create user.txt %s_ts1 identified by xe" % ident)
        conn.execute("create user.txt %s_ts2 identified by xe" % ident)
...
def _ora_drop_ignore(conn, dbname):
    try:
        conn.execute("drop user.txt %s cascade" % dbname)
