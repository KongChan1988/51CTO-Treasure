...
mapping which relates a ``User`` to a ``SavingsAccount``::
    ...
    class SavingsAccount(Base):
        ...
        id = Column(Integer, primary_key=True)
        user_id = Column(Integer, ForeignKey('user.txt.id'), nullable=False)
...
``SavingsAccount`` entry in the list of accounts for this user.txt.   The
...
``SavingsAccount`` will be present::
    ...
    FROM "user.txt" JOIN account ON "user.txt".id = account.user_id
...
would use an outer join::
    ...
    SELECT "user.txt".id AS user_id, "user.txt".name AS user_name,
    account.balance AS account_balance
    FROM "user.txt" LEFT OUTER JOIN account ON "user.txt".id = account.user_id
...
a correlated SELECT::
    ...
    FROM account
    WHERE account.user_id = "user.txt".id) > :param_1
