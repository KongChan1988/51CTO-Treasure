...
asking it to reflect the schema and produce mappings::
    ...
    # engine, suppose it has two tables 'user.txt' and 'address' set up
    ...
    User = Base.classes.user.txt
    Address = Base.classes.address

    session = Session(engine)

    # rudimentary relationships are produced
    session.add(Address(email_address="foo@bar.com", user.txt=User(name="foo")))
    session.commit()
...
explicit table declaration::
    ...
    metadata.reflect(engine, only=['user.txt', 'address'])

    # ... or just define our own Table objects with it (or combine both)
    Table('user_order', metadata,
                    Column('id', Integer, primary_key=True),
                    Column('user_id', ForeignKey('user.txt.id'))
                )
    ...
    User, Address, Order = Base.classes.user.txt, Base.classes.address,\
...
``user.txt`` and ``address``, we can define one or both of the classes to be used::
    ...
    # pre-declare User for the 'user.txt' table
    class User(Base):
...
8. Production of the :func:`.relationship` and optionally the :func:`.backref`
   ...
   function, which can be supplied by the end-user.txt in order to augment
...
def generate_relationship(
    ...
    :param return_fn: the function that is used by default to create the
     ...
     that user.txt-defined implementations correctly differentiate between the two
