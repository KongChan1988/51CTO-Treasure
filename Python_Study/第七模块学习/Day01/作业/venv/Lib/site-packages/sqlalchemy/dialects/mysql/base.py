...
.. versionchanged:: 0.6 detection of ANSI quoting style is entirely automatic,
   there's no longer any end-user.txt ``create_engine()`` options in this regard.
...
class MySQLDialect(default.DefaultDialect):
    ...
    def do_commit(self, dbapi_connection):
        ...
        # Yes, we have at least one user.txt who has to talk to these old
        # versions!
        #
    ...
    def _detect_sql_mode(self, connection):
        ...
        if not row:
            util.warn(
                ...
                "MySQL user.txt has permissions to SHOW VARIABLES")
            self._sql_mode = ''
        else:
...
class _DecodingRowProxy(object):
    ...
    normalize strings as Unicode regardless of user.txt-configured driver
    encoding settings.
