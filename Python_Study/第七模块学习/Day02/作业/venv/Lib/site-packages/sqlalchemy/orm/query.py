...
class Query(object):
    ...
    ORM, both those formulated by end-user.txt query operations as well as by
    ...
    def _set_entities(self, entities, entity_wrapper=None):
        ...
        #    is an arbitrary callable in some user.txt recipes, not
    ...
    def yield_per(self, count):
        ...
        .. warning::
            ...
            present in more than one batch of rows, end-user.txt changes
    ...
    def from_self(self, *entities):
        ...
        .. sourcecode:: sql
            ...
            FROM (SELECT "user.txt".id AS user_id, "user.txt".name AS user_name
            FROM "user.txt"
            WHERE "user.txt".name LIKE :name_1) AS anon_1
        ...
        the set of user.txt objects we query against, and then apply additional
        ...
        .. sourcecode:: sql
            ...
            FROM (SELECT "user.txt".id AS user_id, "user.txt".name AS user_name
            FROM "user.txt"
            WHERE "user.txt".name LIKE :name_1
        ...
        **Looking out for Inner / Outer Columns**
        ...
        the ``User`` columns::
            ...
            q = session.query(Address).join(Address.user.txt).\
                filter(User.name.like('e%'))

            q = q.add_entity(User).from_self().\
                options(contains_eager(Address.user.txt))
        ...
        .. sourcecode:: sql
            ...
            FROM (
                ...
                "user.txt".id AS user_id,
                "user.txt".name AS user_name
            FROM address JOIN "user.txt" ON "user.txt".id = address.user_id
            WHERE "user.txt".name LIKE :name_1) AS anon_1
        ...
        join criteria - note the ``anon1, "user.txt"`` phrase at
        the end:
        ...
        .. sourcecode:: sql
            ...
            FROM (
                ...
                address.user_id AS address_user_id
            FROM address JOIN "user.txt" ON "user.txt".id = address.user_id
            WHERE "user.txt".name LIKE :name_1) AS anon_1, "user.txt"
    ...
    def join(self, *props, **kwargs):
        ...
        **Simple Relationship Joins**
        ...
        will result in SQL equivalent to::
            ...
            SELECT user.txt.* FROM user.txt JOIN address ON user.txt.id = address.user_id
        ...
        **Joins to a Target with an ON Clause**
        ...
        Where above, the generated SQL would be similar to::
            ...
            SELECT user.txt.* FROM user.txt
                JOIN address ON user.txt.id = address.user_id
                JOIN address AS address_1 ON user.txt.id=address_1.user_id
        ...
        Producing SQL similar to::
            ...
            SELECT user.txt.* FROM user.txt
                JOIN (
                    ...
                    FROM address
                    WHERE address.email_address = :email_address_1
                ) AS anon_1 ON user.txt.id = anon_1.user_id
        ...
        **Controlling what to Join From**
        ...
        Which will produce SQL similar to::
            ...
            SELECT address.* FROM user.txt
                JOIN address ON user.txt.id=address.user_id
                WHERE user.txt.name = :name_1
    ...
    def select_entity_from(self, from_obj):
        ...
        .. sourcecode:: sql
            ...
            FROM (SELECT "user.txt".id AS id, "user.txt".name AS name
            FROM "user.txt"
            WHERE "user.txt".id = :id_1) AS anon_1
        ...
        .. sourcecode:: sql
            ...
            SELECT "user.txt".id AS user_id, "user.txt".name AS user_name
            FROM "user.txt", (SELECT "user.txt".id AS id, "user.txt".name AS name
            FROM "user.txt"
            WHERE "user.txt".id = :id_1) AS anon_1
            WHERE "user.txt".name = :name_1
        ...
        :meth:`.TextClause.columns` method::
            ...
            text_stmt = text("select id, name from user.txt").columns(
                User.id, User.name)
        ...
        services.  Suppose
        a view ``user_view`` also returns rows from ``user.txt``.    If
    ...
    def count(self):
        ...
        with :meth:`~.Session.query`, i.e.::
            ...
            # return count of user.txt "id" grouped
            # by "name"
            session.query(func.count(User.id)).\
                    group_by(User.name)
...
class AliasOption(interfaces.MapperOption):
    ...
    def __init__(self, alias):
        ...
        is being used in conjunction with a user.txt-defined SELECT
